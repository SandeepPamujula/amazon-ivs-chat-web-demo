...
const Chat = (props) => {
  ...
  const tokenProvider = async (selectedUsername, isprovider) => {
    const uuid = uuidv4();
    const permissions = isprovider
      ? ['SEND_MESSAGE', 'DELETE_MESSAGE', 'DISCONNECT_USER']
      : ['SEND_MESSAGE'];

      console.log("called from userid", `${selectedUsername}.${uuid}`);

    const data = {
      arn: props.roomDetails.chatRoomId,
      userId: props.username,
      attributes: {
        username: props.username,
        avatar: `${props.avatarUrl.src}`,
      },
  ...
  useEffect( async () => {
    console.log("called effect",props.roomDetails.chatRegion, props.username, props.isProvider, props.avatarUrl );
    setUsername(props.username);
    setProvider(props.isProvider);

    // Instantiate a chat room
    const room = new ChatRoom({
      regionOrUrl: props.roomDetails.chatRegion,
      tokenProvider:async () =>
        await tokenProvider(props.username, props.isProvider, props.avatarUrl),
    });
    // console.log("room",tokenProvider,props.username, props.isProvider, props.AvatarUrl);
    setChatRoom(room);
    console.log(room);
  ...
  const handleUserKick = async (userId) => {
    const request = new DisconnectUserRequest(userId, 'Kicked by provider');
    try {
  ...
  const renderStickerMessage = (message) => (
    <div className='chat-line-sticker-wrapper' key={message.timestamp}>
      ...
      {provider ? renderChatLineActions(message) : ''}
    </div>
  );
      ...
      <div className='main full-width full-height chat-container'>
        <div className='content-wrapper mg-2'>
          <VideoPlayer
            ...
            playbackUrl={props.roomDetails.playbackUrl}
          />
